...
// import { Search, ChevronDown, Download, Printer, Calendar } from 'lucide-react'
...
//     // Count unique employees who ate (allowed scans)
//     const uniqueEaters = new Set(allowedScans.map(s => s.employee_id?._id || s.employee_id))
    
//     // Count absent employees (employees who didn't eat today)
//     const totalEmployees = filteredEmployees.length
//     const ateToday = uniqueEaters.size
    
//     return {
//       served: allowedScans.length,
//       denied: deniedScans.length,
//       absent: totalEmployees - ateToday,
//       total: totalEmployees,
//       uniqueEaters: ateToday
//     }
...
//     // Check if employee ate each meal
//     const meals = { breakfast: false, lunch: false, dinner: false }
    
//     employeeScans.forEach(scan => {
//       const shift = getShiftFromTime(scan.createdAt)
//       if (shift === 'Breakfast') meals.breakfast = true
//       if (shift === 'Lunch') meals.lunch = true
//       if (shift === 'Dinner') meals.dinner = true
//     })
...
//         return empId === emp._id && isScanOnDate(s, dateStr)
//       })
      
//       const hasBreakfast = dayScans.some(s => getShiftFromTime(s.createdAt) === 'Breakfast')
//       const hasLunch = dayScans.some(s => getShiftFromTime(s.createdAt) === 'Lunch')
//       const hasDinner = dayScans.some(s => getShiftFromTime(s.createdAt) === 'Dinner')
...
//             <button onClick={handleExport} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs px-4 py-2.5 rounded-xl transition-all shadow-lg shadow-indigo-900/30">
//               <Download size={16} />
//               Export CSV
//             </button>
//             <button onClick={handlePrint} className={`flex items-center gap-2 text-xs font-semibold px-4 py-2.5 rounded-xl transition-colors ${
...
const Reports = ({ }) => {
  ...
  // ============================================================
  // ACCESS CONTROL - Check if user can view reports
  // ============================================================
  // const canViewReports = user?.permissions?.viewReports || user?.role === 'super_admin'

  // ============================================================
  // SITE ADMIN / MESS KEEPER ACCESS DENIED VIEW
  // ============================================================
  // if (!canViewReports) {
  //   return (
  //     <div className={`min-h-screen p-4 md:p-4 transition-colors duration-300 ${isDarkMode ? 'bg-slate-950 text-white' : 'bg-gray-50 text-gray-900'}`}>
  ...
  //         }`}>
  //           <Lock size={40} className={isDarkMode ? 'text-gray-500' : 'text-gray-400'} />
  //         </div>
  ...
  //           Please contact your Super Admin to enable this permission.
  //         </p>
