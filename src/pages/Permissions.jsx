...
const Permissions = () => {
  ...
  const { unassignedDevices: devices, unassignedLoading: loadingDevices } = useSelector((state) => state.devices)  
  ...
  // Fetch devices whenever the promote target's site changes
  useEffect(() => {
    if (promoteTarget?.siteId) {
      dispatch(fetchUnassignedDevices())
    }
  }, [dispatch, promoteTarget?.siteId])
  ...
  const handleRoleChange = async (userId, newRole) => {
    ...
    if (u.source !== 'user') {
      showError('This is an employee. Use "Promote" instead.')
      return
    }

    const ok = await confirm({
      title: 'Change Role',
      message: newRole === 'employee'
        ? `Demote ${u.name} to Employee? They will lose all access.`
        : `Change ${u.name}'s role to ${newRole.replace('_', ' ')}?`,
      confirmText: 'Change',
      variant: newRole === 'employee' ? 'warning' : 'default',
    })
  ...
  const handlePromoteConfirm = async ({ password, role, device_serial }) => {
    ...
    try {
      ...
      const roleLabel = (role || 'site_admin').replace('_', ' ')
      showSuccess(`${result.name} promoted to ${roleLabel}`)
    ...
    <div className={`min-h-screen p-3 sm:p-4 transition-colors duration-300 ${isDarkMode ? 'bg-slate-950 text-white' : 'bg-gray-50 text-gray-900'}`}>
      ...
      <div className={`rounded-xl border p-3 sm:p-4 mb-4 flex items-start gap-3 ${
        ...
        <p className={`text-xs sm:text-sm ${isDarkMode ? 'text-amber-500' : 'text-amber-700'}`}>
          Grant page-level access to Site Admins. Promote employees to Site Admins (web access) or Mess Keepers (mobile scanner only).
        </p>
      </div>
      ...
      <div className={`inline-flex rounded-xl p-1 mb-4 overflow-x-auto max-w-full ${
        ...
        {['All', 'super_admin', 'site_admin', 'mess_keeper', 'employee'].map((tab) => (
          <button
            key={tab}
        ...
        <div className="space-y-4">
          {users.length === 0 ? (
            ...
            users.map((u) => {
              ...
              const isMessKeeper = u.role === 'mess_keeper' && u.source === 'user'
              ...
              return (
                <div key={u._id} className={`rounded-xl border p-3 sm:p-4 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-gray-200'}`}>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4 mb-4">
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {isMessKeeper && u.device_serial && (
                          <span className={`inline-block mt-1 ml-1 text-[11px] px-2 py-0.5 rounded ${isDarkMode ? 'bg-amber-500/10 text-amber-400' : 'bg-amber-50 text-amber-600'}`}>
                            Device: {u.device_serial}
                          </span>
                        )}
                      </div>
                    </div>
                      ...
                      {(isSiteAdmin || isMessKeeper) && (
                        <div className="flex items-center gap-2 w-full sm:w-auto">
                          ...
                          >
                            ...
                            <option value="mess_keeper">Mess Keeper</option>
                  ...
                  {isMessKeeper && (
                    <div className={`rounded-xl p-3 text-xs border ${isDarkMode ? 'bg-amber-500/5 border-amber-500/20 text-amber-400' : 'bg-amber-50 border-amber-200 text-amber-700'}`}>
                      <div className="flex items-center gap-2">
                        <QrCode size={14} />
                        <span>
                          Mess Keeper — mobile scanner app only.
                          {u.device_serial
                            ? ` Locked to device ${u.device_serial}.`
                            : ' No device assigned — they will pick one on first launch.'}
                        </span>
                      </div>
                    </div>
                  )}
                  ...
                  {isEmployee && (
                    <div className={`rounded-xl p-3 text-xs border ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-gray-50 border-gray-200'}`}>
                      <p className={`mb-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                        This employee doesn't have a user account yet. Promote them to Site Admin or Mess Keeper to grant access.
                      </p>
