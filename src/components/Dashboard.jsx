...
//       value: activeSites,
//       icon: MapPin,
//       subtext: inactiveSites === 0 ? 'All active' : `${inactiveSites} inactive`,
//       trend: activeSites > 0 ? 'up' : 'down',
//     },
//     {
...
//       trend: allowedScans > 0 ? 'up' : 'down',
//     },
//     {
//       title: 'Devices Online',
//       value: `${onlineDevices} / ${totalDevices}`,
//       icon: Monitor,
//       subtext: offlineDevices === 0 ? 'All online' : `${offlineDevices} offline`,
//       trend: onlineDevices > 0 ? 'up' : 'down',
//     },
//   ]
...
const Dashboard = ({ }) => {
  ...
  // Auto-refresh devices every 30 sec so online status stays fresh
  useEffect(() => {
  ...
  // Devices — use isOnline from heartbeat
  const onlineDevices = devices.filter(d => d.isOnline === true).length
  const totalDevices = devices.length
  const offlineDevices = totalDevices - onlineDevices

  // Active sites = unique sites with at least one online device
  ...
  const stats = [
    ...
    },
    {
      ...
      value: totalSites,
      icon: MapPin,
      subtext: activeSites === totalSites && totalSites > 0
        ? 'All active'
        : `${activeSites} active`,
      trend: activeSites > 0 ? 'up' : 'down',
    },
    ...
    },
    {
      ...
      value: `${onlineDevices} / ${totalDevices}`,
      icon: Monitor,
      subtext: totalDevices > 0 && offlineDevices === 0
        ? 'All online'
